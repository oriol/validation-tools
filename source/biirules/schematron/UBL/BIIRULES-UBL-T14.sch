...
<pattern xmlns="http://purl.oclc.org/dsdl/schematron" is-a="T14" id="UBL-T14">
  ...
  <param name="BII2-T14-R051" value="(xs:decimal(cbc:LineExtensionAmount)) = (round(sum(xs:decimal(//cac:CreditNoteLine/cbc:LineExtensionAmount)) * 10 * 10) div 100)"/>
  ...
  <param name="BII2-T14-R054" value="(xs:decimal(cbc:AllowanceTotalAmount)) = (round(sum(xs:decimal(/ubl:CreditNote/cac:AllowanceCharge[cbc:ChargeIndicator=&quot;false&quot;]/cbc:Amount)) * 10 * 10) div 100) or not(cbc:AllowanceTotalAmount)"/>
  <param name="BII2-T14-R055" value="(xs:decimal(cbc:ChargeTotalAmount)) = (round(sum(xs:decimal(/ubl:CreditNote/cac:AllowanceCharge[cbc:ChargeIndicator=&quot;true&quot;]/cbc:Amount)) * 10 * 10) div 100) or not(cbc:ChargeTotalAmount)"/>
