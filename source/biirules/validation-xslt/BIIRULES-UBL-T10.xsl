...
<xsl:stylesheet xmlns:xs="http://www.w3.org/2001/XMLSchema"
   ...
   <!--PATTERN UBL-T10-->
	  ...
	  <!--RULE -->
   <xsl:template match="/ubl:Invoice/cac:AllowanceCharge" priority="1006" mode="M6">
	  ...
	  <!--RULE -->
   <xsl:template match="/ubl:Invoice" priority="1005" mode="M6">
	  ...
	  <!--RULE -->
   <xsl:template match="//cac:InvoiceLine" priority="1004" mode="M6">
      ...
      <xsl:choose>
         ...
         <xsl:otherwise>
		    ...
		    <!--ASSERT -->
      <xsl:choose>
         <xsl:when test="(cac:Item/cac:ClassifiedTaxCategory/cbc:ID) or not(/ubl:Invoice/cac:TaxTotal/*/*/*/cbc:ID='VAT')"/>
         <xsl:otherwise>
            <svrl:failed-assert xmlns:svrl="http://purl.oclc.org/dsdl/svrl"
	  ...
	  <!--RULE -->
   <xsl:template match="//cac:InvoicePeriod" priority="1003" mode="M6">
	  ...
	  <!--RULE -->
   <xsl:template match="//cac:PaymentMeans" priority="1002" mode="M6">
   ...
   <xsl:template match="//cac:LegalMonetaryTotal" priority="1001" mode="M6">
      <svrl:fired-rule xmlns:svrl="http://purl.oclc.org/dsdl/svrl"
		    ...
		    <!--ASSERT -->
      <xsl:choose>
         <xsl:when test="(xs:decimal(cbc:LineExtensionAmount)) = (round(xs:decimal(sum(//cac:InvoiceLine/cbc:LineExtensionAmount)) * 10 * 10) div 100)"/>
         <xsl:otherwise>
            <svrl:failed-assert xmlns:svrl="http://purl.oclc.org/dsdl/svrl"
                                test="(xs:decimal(cbc:LineExtensionAmount)) = (round(xs:decimal(sum(//cac:InvoiceLine/cbc:LineExtensionAmount)) * 10 * 10) div 100)">
   ...
   <!--PATTERN CodesT10-->
	  ...
	  <!--RULE -->
   <xsl:template match="cbc:InvoiceTypeCode" priority="1007" mode="M7">
